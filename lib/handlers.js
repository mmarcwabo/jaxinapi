...
handlers.index = function (data, callback) {
    ...
    if (data.method == 'get') {
        ...
        var templateData = {
            ...
            'head.description': 'Free uptime monitoring for HTTP/HTTPS sites of all the kinds. When your site goes down, we ll text you.',
            'body.class': 'index',
        };
...
handlers.public = function (data, callback) {
    ...
    if (data.method == 'get') {
        ...
        var trimAssetName = data.trimmedPath.replace('public/', '').trim();
        if (trimAssetName.length > 0) {
            ...
            helpers.getStaticAsset(trimAssetName, function (err, data) {
                if (!err && data) {
                    ...
                    if(trimAssetName.indexOf('.css') > -1) {
                        contentType = 'css';
                    }
                    if(trimAssetName.indexOf('.png') > -1) {
                        contentType = 'png';
                    } 
                    if(trimAssetName.indexOf('.jpg') > -1) {
                        contentType = 'jpg';
                    } 
                    if(trimAssetName.indexOf('.ico') > -1) {
                        contentType = 'favicon';
                    } 
                    // Call back the data
